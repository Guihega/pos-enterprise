...
- **[deuda-21] RETIRADA: hallazgo FALSO** (2026-08-21, mismo dia de su
  declaracion). Verificado con `grep -A3` sobre los once `Rule::unique`
  de los Update: TODOS llevan `->where('company_id')` e `->ignore()`
  (UpdateCustomerRequest ademas `whereNull('deleted_at')`). La
  declaracion se baso en salida de grep TRUNCADA (reglas multilinea
  cortadas en la primera linea): la falta metodologica que la regla "un
